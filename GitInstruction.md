...
1. На Github делаем Fork(Fork(ответвление) - это копия репозитория.Это тоже самое, что branch в Git) репозитория для этого:
   * заходите на GitHub под своей учетной записью
   * переходите по ссылке на которой расположен репозиторий
   * В правом верхнем углу окна браузера находите и нажимайте кнопку Fork.
   * Теперь этот репозиторий Ваш - точнее, у меня теперь копия оригинального репозитория
